...
class Config(commands.Cog):
    ...
    async def help_command(self, interaction: discord.Interaction):
        ...
        mod_cmds = [
            ...
            "`/tempban` - Temporarily ban (auto-unban)",
            "`/kick` - Kick a user",
            "`/mute` - Mute a user (temp/permanent)",
            "`/unmute` - Unmute a user",
            "`/timeout` - Timeout user (Discord native)",
            "`/warn` - Issue warning",
        ...
        economy_cmds = [
            ...
            "`/rob` - Rob users (40% success)",
            "`/coinflip` - Bet on coinflip",
            "`/slots` - Slot machine (up to x10)",
            "`/leaderboard` - Top richest users",
            "`/setcurrency` - Set custom currency"
        ]
        
        # Ticket Commands
        ticket_cmds = [
            "`/ticketpanel` - Create advanced panel with categories",
            "`/adduser` - Add user to ticket",
            "`/close` - Close ticket (with reason)",
            "`/transcript` - Generate transcript",
            "`/ticketblacklist` - Blacklist users",
        ...
        config_cmds = [
            ...
            "`/setfooter` - Customize embed footer",
            "`/help` - Show this message"
        ]
        
        # Add fields to the embed
        embed.add_field(name="🔨 Moderation (15)", value="\n".join(mod_cmds), inline=False)
        embed.add_field(name="💰 Economy (10)", value="\n".join(economy_cmds), inline=False)
        embed.add_field(name="🎫 Tickets (6)", value="\n".join(ticket_cmds), inline=False)
        embed.add_field(name="🛠️ Utility (10)", value="\n".join(utility_cmds), inline=False)
        embed.add_field(name="⚙️ Configuration (4)", value="\n".join(config_cmds), inline=False)
        
        # Footer
        embed.set_footer(text=f"Total: 45 commands | Use /command to execute")
